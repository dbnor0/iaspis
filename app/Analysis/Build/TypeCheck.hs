...
module Analysis.Build.TypeCheck where
...
typeCheck Module{ moduleDecl, declarations } = withScope biModule moduleDecl $ traverse_ typeCheckDecl declarations

typeCheckDecl :: MonadState BuildEnv m => MonadError BuildError m => Declaration -> m ()
typeCheckDecl = \case
  ContractDecl c -> typeCheckContract c
  FacetDecl f -> typeCheckFacet f
  _ -> return ()
...
typeCheckFacet (FacetContract _ _ fns) = return () -- traverse_ typeCheckFn fns 
